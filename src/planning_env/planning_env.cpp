...
void PlanningEnvParameters::ReadParameters(ros::NodeHandle& nh)
{
  ...
  int viewpoint_number = misc_utils_ns::getParam<int>(nh, "viewpoint_manager/number_x", 10);
  ...
  double sensor_range = misc_utils_ns::getParam<double>(nh, "kSensorRange", 5);
  ...
  , robot_position_update_(false)
{
  ...
  rolling_free_occupancy_cloud_ = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
      nh, "rolling_free_occupancy_cloud", world_frame_id);
